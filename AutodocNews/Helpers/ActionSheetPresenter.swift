...
struct ActionSheetPresenter {
    static func presentShareActions(
        ...
        let copyAction = UIAlertAction(title: GlobalConstants.copyLink, style: .default) { _ in
            UIPasteboard.general.string = url
        }
        copyAction.setValue(UIImage(systemName: "doc.on.doc"), forKey: "image")

        let shareAction = UIAlertAction(title: GlobalConstants.shareLink, style: .default) { _ in
        ...
        let cancelAction = UIAlertAction(title: GlobalConstants.cancel, style: .cancel)
